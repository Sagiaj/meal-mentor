...
const actions = {
    ...
    async authUserLogin({ commit, dispatch }: any, { email, password }: UserCredentials): Promise<FirebaseUser> {
        try {
    ...
    async authUserRegister({ commit, dispatch }: any, { email, password, username }: UserCredentials): Promise<FirebaseUser> {
        try {
            let user: FirebaseUser = await FirebaseService.signUp(email, password, username);
            commit(types.SET_USER, user);
