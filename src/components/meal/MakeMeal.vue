<template>
  <v-container grid-list-xl>
    ...
    >
      <v-container fluid fill-height style="background-color: rgba(0, 0, 0, 0.7);">
    ...
    <v-layout row wrap text-xs-center justify-center align-center>
      <v-flex xs4 v-for="(foodType, idx) in ingredients" :key="idx">
        <v-autocomplete
      ...
      <v-layout row wrap text-xs-center justify-center v-if="mealComplete">
        ...
        >
          <v-progress-circular
            ...
            :color='["red darken-3", "red", "warning", "yellow darken-2", "blue darken-3", "teal", "success"][Math.floor(mealScore[ingredient.name] / 15)]'
            :width="24"
          >
            <h3>{{ ['Terrible', 'Bad', 'Average', 'Good', 'Great', 'Excellent', 'Perfect'][Math.floor(mealScore[ingredient.name] / 15)] }}</h3>
          </v-progress-circular>
        </v-flex>
        <v-flex xs12>
          ...
          <v-progress-circular
            ...
            :color='["red darken-3", "red", "warning", "yellow darken-2", "blue darken-3", "teal", "success"][Math.floor(mealScore.total / 15)]'
            :width="30"
          >
          <h2>
            {{ ['Terrible', 'Bad', 'Average', 'Good', 'Great', 'Excellent', 'Perfect'][Math.floor(mealScore.total / 15)] }}
          </h2>
          </v-progress-circular>
        </v-flex>
        <v-flex xs12>
...
export default {
  ...
  computed: {
    ...
    mealScore() {
      if (!this.mealComplete) return { proteins: 0, carbs: 0, fats: 0, total: 0 };
      ...
      // };
      let fatScore, protScore, carbScore,
      ...
      if (Math.abs(this.scores.fats - this.allowedCalories.fats) >= fatDev) {
        fatScore = 115 * Math.min(this.allowedCalories.fats, this.scores.fats) / Math.max(this.allowedCalories.fats, this.scores.fats);
      } else {
        fatScore = 100;
      }

      if (Math.abs(this.scores.carbs - this.allowedCalories.carbs) >= carbDev) {
        carbScore = 115 * Math.min(this.allowedCalories.carbs, this.scores.carbs) / Math.max(this.allowedCalories.carbs, this.scores.carbs);
      } else {
        carbScore = 100;
      }

      if (Math.abs(this.scores.proteins, this.allowedCalories.proteins) >= protDev) {
        protScore = 115 * Math.min(this.allowedCalories.proteins) / Math.max(this.allowedCalories.proteins, this.scores.proteins);
      } else {
        protScore = 100;
      }
      ...
      return {
        ...caloricScores,
        total: Math.min(100, Math.floor((caloricScores.proteins + caloricScores.carbs + caloricScores.fats) / 3))
      };
    }
    // color() {
  ...
  methods: {
    async optimizeByConstraints() {
      ...
      if (this.checkConstraints() || (+new Date() - this.calculationStartTime > 500)) {
